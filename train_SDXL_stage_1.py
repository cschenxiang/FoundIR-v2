...
from foundirv2.pipelines.pipeline_foundirv2_tlc import foundirv2StableDiffusionXLPipeline
from foundirv2.models.unet_2d_condition_vae_extension import UNet2DConditionModel
from foundirv2.training_utils import EMAModel
...
def log_validation(unet, args, accelerator, weight_dtype, step):
    ...
    pipe = foundirv2StableDiffusionXLPipeline.from_pretrained(
    ...
    test_img_list = os.listdir('./validation')[:10]
    img_count = 0
    images = []
    for file_name in test_img_list:
        
        data = json.load(open(os.path.join('./caption',file_name.split('.')[0]+'.json')))
        lr_image = Image.open(os.path.join('./validation',file_name)).convert('RGB')
        ...
        gen_image = pipe(lr_img=lr_image, prompt = prompt_init, negative_prompt = negative_prompt_init, num_images_per_prompt=num_samples, start_point='noise', num_inference_steps=20, seed=42,height = 1024, width=1024).images[0]
...
def main():
    ...
    # optimizer
    params_to_opt = itertools.chain(unet.condition_embedding.parameters(), unet.MoE.parameters(), unet.spatial_ch_projs.parameters())

    optimizer_grouped_parameters = [{'params': params_to_opt, 'lr': args.learning_rate, 'weight_decay': args.adam_weight_decay, 'betas': (args.adam_beta1, args.adam_beta2), 'eps':args.adam_epsilon}]
    ...
    img_file_path = './dataset/HQ_folder/'

    yml_kernel = './train_kernel.yml'

    train_dataset = LocalImageDataset(img_file_path,yml_kernel=yml_kernel, image_size=args.resolution, tokenizer=tokenizer,tokenizer_2=tokenizer_2, t_drop_rate=0.2)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("foundirv2_stage_1", config=vars(args))
